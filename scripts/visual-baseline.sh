...
if [ ! -f "$CONFIG" ]; then
  echo "visual-baseline: $CONFIG not found in $REPO_ROOT" >&2
  exit 1
fi
...
if ! curl -sf "http://localhost:$PORT" >/dev/null 2>&1; then
  echo "visual-baseline: starting dev server ($DEV_COMMAND) on port $PORT..."
  eval "$DEV_COMMAND" >/tmp/visual-baseline-devserver.log 2>&1 &
  DEV_PID=$!
  ...
  until curl -sf "http://localhost:$PORT" >/dev/null 2>&1; do
    ...
    if [ "$ELAPSED" -ge "$READY_TIMEOUT_MS" ]; then
      echo "visual-baseline: dev server did not become ready within ${READY_TIMEOUT_MS}ms" >&2
      cat /tmp/visual-baseline-devserver.log >&2
      exit 1
    fi
  done
else
  echo "visual-baseline: dev server already running on port $PORT, reusing it"
fi

echo "visual-baseline: capturing baseline screenshots..."
node "$SCRIPT_DIR/lib/capture.mjs" --mode=baseline --config="$CONFIG" --out=visual/baseline

echo "visual-baseline: done. Baseline written to visual/baseline/"
